...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val searchText = findViewById<TextView>(R.id.editTextSearch)
        ...
        val contacts = Gson().fromJson<List<Contact>>(phones, phonesType)
        adapter.submitList(contacts)

        searchBtn.setOnClickListener{

            val filteredContacts = contacts.filter { item ->
                item.phone.lowercase().contains(searchText.text) || item.name.lowercase().contains(searchText.text) || item.type.lowercase().contains(searchText.text)
            }
